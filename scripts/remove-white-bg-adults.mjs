...
const dirs = [
  'public/images/products/adult-diapers-30',
  'public/images/products/adult-diapers-10',
];

for (const dir of dirs) {
  const abs = path.resolve(dir);
  const files = await fs.readdir(abs);
  const jpgs = files.filter((f) => /\.jpg$/i.test(f) && /ADULT_DIAPERS/i.test(f));
  ...
  for (const file of jpgs) {
    ...
    // Near white → transparent via flood fill from edges
    const isWhiteLike = (i) => {
      const r = buf[i];
      const g = buf[i + 1];
      const b = buf[i + 2];
